...
(function () {
    ...
    angular.module("myapp.controllers", ['ionic-datepicker', 'tabSlideBox', 'ngAnimate'])
        ...
        .controller("parentReportsCtrl", ["$scope", "$http", "$state", "$ionicLoading", "$CustomLS", function ($scope, $http, $state, $ionicLoading, $CustomLS) {
            $scope.Reports = [];
            $scope.groupedReport = undefined;
            var studentId = localStorage['selectedStudent']
            $scope.OrgId = localStorage['selectedStudentOrgId'];
            $scope.content = "";
            $ionicLoading.show({
                template: 'Loading Reports...',
                duration: 20000
            });
            $http.get(host + '/Report/getReportBaseDetails?StudentId=' + studentId + '&OrgId=' + $scope.OrgId).success(function (data) {
                ...
                $scope.groupedReport = [];
                //getting unique date to group the reports
                var uniqueReportDates = getUniqueDates(data.map(function (e) { return e.createdOn; }));
                uniqueReportDates.forEach(function (e, i) {
                    $scope.groupedReport.push({
                        Items: data.filter(function (f) { return compareOnlyDate(f.createdOn, e); }).sort(function (a, b) { return a.createdOn < b.createdOn; }),
                        Date: e.toLocaleDateString() == (new Date()).toLocaleDateString() ? "Today" : e.toLocaleDateString()
                    });
                });
            });
        }])
